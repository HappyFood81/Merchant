...
let navFood = document.getElementById("navFood");
let navRider = document.getElementById("navRider");
...
userBox.style = `height:${headerHeight};width:100%`;


function user(val) {
    if (val) userBox.style.top = `${headerHeight}`;
    else userBox.style.top = `0`;
}
...
function navThemeFunc() {
    if (!navThemeBoolean) {
        ...
        document.querySelector(".user-box").className += " black-grey-border";
        ...
        document.querySelector(".user-box").className = "user-box";
...
if (localStorage.getItem("visit") == 2) {
    introLoading.style.visibility = "hidden";
} else {
    introLoad();
    localStorage.setItem("visit", 2);
    numVisit = localStorage.getItem("visit");
}
...
dataCustomer().then(d => {
    if (d.length === 0) {
        ...
        for (i = 0; i < d.length; i++) {
            ...
            const id = info._id;
            const order = info.orders;
            
            const name = info.name;
            const phone = info.phone;
            const address = info.house;
            const resto = info.resto;
            const subTotal = info.subTotal;
            const email = info.email;
            const time = info.time.substr(0, 7);
            const delFee = info.delFee;

            const newOrder = info.newOrder;
            const preparing = info.preparing;
            const finished = info.finished;

            const t = new Date();
            const hour = t.getHours();
            const minutes = t.getMinutes();
            let foodNumber = Number(`${hour}${minutes}`);
    ...
    if (darkMode) {
        for (i = 0; i < allNewOrder.length; i++) {
            ...
            <div class="section-box new white-shadow">
                <div class="section-box-stc">#224 Order</div>
...
function seeNewOrder(i) {
    orderShowcase.style = "visibility:visible;opacity:1;";
    orderContainer.style = "visibility:visible;opacity:1;";
        ...
        if (orders.add.length > 3 && orders.flavor.length > 3) {
            ...
            <div class="order-box">
                ...
                <div class="order-box-des2">
                    ...
                    <div class="order-box-des2-btn"><button>Remove</button></div>
                </div>
            </div>
            `;
...
setInterval(() => {
    newOrders.innerHTML = allNewOrder.length;
    preparing.innerHTML = allPrepOrder.length;
    finsOrders.innerHTML = allFinsOrder.length;
}, 1000);
...
// 
// `);
// document.getElementById("sectionWrapper1").innerHTML;